package com.ty.mid.framework.web.annotation.desensitize.serializer;

import cn.hutool.core.collection.CollUtil;
import cn.hutool.core.lang.Assert;
import com.fasterxml.jackson.core.JsonGenerator;
import com.fasterxml.jackson.databind.JsonSerializer;
import com.fasterxml.jackson.databind.SerializerProvider;
import com.ty.mid.framework.common.exception.FrameworkException;
import com.ty.mid.framework.common.util.AssertUtil;
import com.ty.mid.framework.common.util.HashIdUtil;
import com.ty.mid.framework.core.spring.SpringContextHelper;
import com.ty.mid.framework.web.config.WebConfig;
import lombok.Data;
import lombok.EqualsAndHashCode;
import lombok.NoArgsConstructor;
import org.apache.commons.collections.CollectionUtils;

import java.io.IOException;
import java.util.*;
import java.util.stream.Collectors;

/**
 * Json数据出参处理
 * 注:支持
 * 1.Long类型
 * 2.? extends Collection<Long> 类型
 * 3.String类型,实际值为Long类型
 */
@EqualsAndHashCode(callSuper = true)
@Data
@NoArgsConstructor
public class HashedIdJsonSerializer extends JsonSerializer<Object> {

    private WebConfig.HashId hashIdConfig = SpringContextHelper.getBean(WebConfig.class).getHashId();

    @Override
    public void serialize(Object value, JsonGenerator gen, SerializerProvider serializers) throws IOException {
        if (Objects.isNull(value)){
            return;
        }
        if (!hashIdConfig.isEnable()) {
            gen.writeString(value.toString());
            return;
        }
        Assert.isTrue(value.getClass().isAssignableFrom(Long.class) || value.getClass().isAssignableFrom(Collection.class)
                || value.getClass().isAssignableFrom(String.class),"当前类型:{},仅支持Long,集合,或类型为String的Long数据使用HashId注解",value.getClass() );
        String out = "";
        if (value.getClass().isAssignableFrom(Long.class)){
            out = HashIdUtil.encode((Long) value, hashIdConfig.getSalt(), hashIdConfig.getMinLength());
            gen.writeString(out);
            return;
        }
        //兼容类型为String,实际数据为Long的情况
        if (value.getClass().isAssignableFrom(String.class)){
            try {
                long longValue = Long.parseLong((String) value);
                out = HashIdUtil.encode(longValue, hashIdConfig.getSalt(), hashIdConfig.getMinLength());
                gen.writeString(out);
                return;
            }catch (NumberFormatException e){
                throw new FrameworkException("类型为String,实际应为Long数据才可以使用HashId注解");
            }
        }
        //兼容集合HashId加密
        if (value.getClass().isAssignableFrom(Collection.class)){
            Collection<?> collection = (Collection<?>) value;
            if (CollUtil.isNotEmpty(collection)){
                List<String> result = collection.stream().map(innerValue -> {
                    Assert.isTrue(innerValue.getClass().isAssignableFrom(Long.class), "集合中仅Long数据可使用HashId注解");
                    return HashIdUtil.encode((Long) innerValue, hashIdConfig.getSalt(), hashIdConfig.getMinLength());
                }).collect(Collectors.toList());
                String[] array = result.toArray(new String[0]);
                gen.writeArray(array,0,result.size());
            }
        }


    }
}
